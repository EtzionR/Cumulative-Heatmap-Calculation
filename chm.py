...
class HeatMap:
    ...
    def __init__(self, data, division):
        ...
        self.xy = np.array(data)
        self.x = self.xy[:,0]
        self.y = self.xy[:,1]
        self.x_min, self.y_min = np.min(self.x), np.min(self.y)
        self.x_max, self.y_max = np.max(self.x), np.max(self.y)
    ...
    def square_intersection(self):
        ...
        """
        cells = np.column_stack(((self.x-self.x_min)//self.side,
                                 (self.y-self.y_min)//self.side))
        keys,values = np.unique(cells, axis=0, return_counts=True)
        return {(keys[i][0],keys[i][1]):values[i] for i in range(len(values))}
    ...
    def plot(self,size=6):
        ...
        p = PatchCollection(poly, cmap=MAP)
